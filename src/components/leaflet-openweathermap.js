...
L.OWM = L.TileLayer.extend({
  options: {
    appId: 'a22e7429bbe086045388e31142cd915f', /* pass your own AppId as parameter when creating the layer. Get your own AppId at https://www.openweathermap.org/appid */
...
L.OWM.Current = L.Layer.extend({
    ...
    if (this.options.progressControl) {
      ...
      if (this.options.imageLoadingBgUrl) {
        ...
        if (this.options.type !== 'city') {
          var bgIconCity = this.options.imageUrlStation;
        }
      }
  ...
  _processRequestedData: function(_this, data) {
    ...
    for (var i in data) {
      ...
      if (!stations[key] || parseInt(stations[key].rang, 10) < parseInt(stat.rang, 10)) {
        stations[key] = stat;
      }
    }
