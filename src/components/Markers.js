...
export class Markers extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    return {
      callsign: nextProps.selectedFlight
        ? nextProps.selectedFlight.callsign
        : null
    };
  }
  ...
  render = () => {
    ...
    return flights.map((position, idx) => {
      const {
        altitude,
        callsign,
        frequency,
      ...
      return (
        <RotatedMarker
          ...
          onClick={e => {
            if (!isController) {
              e.sourceTarget.setIcon(
                this.setSelected(planned_aircraft, callsign)
              );
        ...
        >
          <Tooltip direction="auto">
            <Fragment>
              ...
              <div>{name}</div>
              {isController && <div>{frequency}</div>}
              {!isController && (
                <Fragment>
